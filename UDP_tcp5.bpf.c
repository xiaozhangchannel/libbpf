...
//定义 map
struct bpf_map_def SEC("maps") udp_flow_map = {
	.type = BPF_MAP_TYPE_HASH,
	.key_size = sizeof(unsigned int ),
	.value_size = sizeof(struct data_t),
	.max_entries = 1024,
};
//使用 kprobe 挂载 udp_sendmsg 函数
SEC("kprobe/udp_sendmsg")
int bpf_prog1(struct pt_regs *ctx)
{
	unsigned int key = bpf_get_current_pid_tgid() >> 32;
	unsigned int tmp = (unsigned int)PT_REGS_PARM3(ctx);
	struct data_t *value = bpf_map_lookup_elem(&udp_flow_map, &key);
	if (value)
		value->send += tmp;
	else {
		struct data_t val = {.send = tmp, .recv = 0};
		bpf_map_update_elem(&udp_flow_map, &key, &val, BPF_ANY);
	}
	return 0;
}
//使用 kretprobe 挂载 udp_recvmsg 函数
SEC("kretprobe/udp_recvmsg")
int bpf_prog2(struct pt_regs *ctx)
{
	unsigned int key = bpf_get_current_pid_tgid() >> 32;
	unsigned int tmp = (int)PT_REGS_RC(ctx);
	if (tmp <= 0)
		return 0;
	struct data_t *value = bpf_map_lookup_elem(&udp_flow_map, &key);
	if (value)
